///@file Decompose.cpp
///@brief Implementation of the Decompose algorithm for polygon triangulation.
///
/// The Decompose algorithm is a variant of the ear-clipping method that is used to decompose a polygon into a set of triangles.
...
/// The Decompose algorithm works by finding a sequence of vertices that form a simple polygon, and then removing inessential diagonals until the polygon is completely triangulated.
///
...
Decompose::Decompose(DCEL *&dcel)
{
    notches = getAllNotches(dcel->faces[EXTERNAL_FACE]);
    polygon = dcel;
}

bool Decompose::primeDecompose()
{
    L.clear();
